...
{
  unsigned int i;
  unsigned char coins[3*CTRU_N/2];

  do
  {
    randombytes(coins, CTRU_SEEDBYTES);
    crypto_hash_shake128(coins, 3*CTRU_N/2, coins, CTRU_SEEDBYTES);
...
{
  unsigned int i;
  unsigned char buf[CTRU_SHAREDKEYBYTES + 3*CTRU_N/4], m[CTRU_PREFIXHASHBYTES + CTRU_MSGBYTES];

  randombytes(buf, CTRU_SEEDBYTES);
  crypto_hash_shake128(m + CTRU_PREFIXHASHBYTES, CTRU_MSGBYTES, buf, 32);
  for (i = 0; i < CTRU_PREFIXHASHBYTES; ++i)
    m[i] = pk[i];
  crypto_hash_sha3512(buf, m, CTRU_PREFIXHASHBYTES + CTRU_MSGBYTES);
  crypto_hash_shake128(buf + CTRU_SHAREDKEYBYTES, 3*CTRU_N/4, buf + CTRU_SHAREDKEYBYTES, 32);
...
{
  unsigned int i;
  unsigned char buf[CTRU_SHAREDKEYBYTES + 3*CTRU_N/4], buf2[CTRU_SHAREDKEYBYTES * 2], m[CTRU_PREFIXHASHBYTES + CTRU_MSGBYTES];
